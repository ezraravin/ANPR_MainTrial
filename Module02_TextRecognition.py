...
def funcExtractText(param_ROIFileName, paramModelEAST, paramModelCRNN):

    # Read Image
    textList = []
    img = cv2.imread(param_ROIFileName)
    ...
    ## -------------  Forward Propagation  -------------
    paramModelEAST.setInput(blob)
    (geometry, scores) = paramModelEAST.forward(paramModelEAST.getUnconnectedOutLayersNames())
    ## -------------   Post-Processing  ----------------
    rectangles = []
    confidence_score = []
    ...
    for (x1, y1, x2,  y2) in fin_boxes:
        ...
        paramModelCRNN.setInput(blob)
        scores = paramModelCRNN.forward()
